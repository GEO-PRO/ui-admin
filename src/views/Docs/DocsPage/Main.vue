<template>
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">Quản lý tài liệu</h2>
        ...
        <div class="text-center mt-5">
          ...
          <div class="mb-5">
            ...
            <div class="text-left mb-5">
              ...
              >
              <TomSelect
                v-model="inputValue.title_doc_id"
                :options="{
                  placeholder: 'Chọn chủ đề',
                }"
                class="w-full"
              >
                <option :value="inputValue.title_doc_id">
                  {{ inputValue.title_name }}
                </option>
                <option
                  v-for="title in titleDocOption"
                  :key="title"
                  :value="title.id"
                >
                  {{ title.name }}
            ...
            <div class="text-left mb-5">
              ...
              >
              <TomSelect
                v-model="inputValue.link_tag_doc"
                :options="{
                ...
                multiple
              >
                <option v-for="tag in tagDocOption" :key="tag" :value="tag.id">
                  {{ tag.name }}
                </option>
              </TomSelect>
            </div>
...
const tagDocOption = ref([]);
const titleDocOption = ref([]);
...
const handleGetEdit = (id) => {
  ...
  axios({
    ...
    .then((result) => {
      let convertData = {
        ...result.data[0],
        link_tag_doc: convertIdArr(result.data[0].link_tag_doc),
      };
...
onMounted(() => {
  ...
  axios({
    url: `${BASE_URL}/tag-doc/get-all`,
    method: "GET",
  })
    .then((result) => {
      result.data.forEach((val) => {
        tagDocOption.value.push(val);
      });
    })
    .catch((err) => {
      console.log("err :>> ", err);
  ...
  axios({
    url: `${BASE_URL}/title-doc/get-all`,
    method: "GET",
  })
    .then((result) => {
      result.data.forEach((val) => {
        titleDocOption.value.push(val);
      });
    })
    .catch((err) => {
      console.log("err :>> ", err);
